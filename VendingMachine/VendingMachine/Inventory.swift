...
struct Inventory {
    ...
    static let chocoFlavoredMilk = ChocoFlavoredMilk("달콤한정훈", 250, 1700, "맛좋은초코유유", "20180301", .secondGrade, 200, 1)
    ...
    static let bananaFlavord = Banana("신선한정훈", 250, 1500, "맛좋은바나나우유", "20180321", .secondGrade, 72, 0.7)
...
extension Inventory: AdminModable {
    ...
    @discardableResult mutating func subtract(productIndex: Int) throws -> Beverage {
        let listOfDrink = drinkLists()
        guard listOfDrink.indices.contains(productIndex - 1) else {
            throw StockError.invalidProductNumber
        }
        let deleteDrink = listOfDrink[productIndex - 1]
        for drink in beverages.enumerated() where drink.element === deleteDrink {
...
extension Inventory: UserModable {
    ...
    mutating func buyBeverage(productIndex: Int) throws -> Beverage {
        ...
        for beverage in beverages where beverage == purchasedBeverage {
