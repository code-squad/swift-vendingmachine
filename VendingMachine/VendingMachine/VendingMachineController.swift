...
class VendingMachineController {
    ...
    func selectFunctionAdministratorMode() {
        ...
        let functions = [
            ...
            Message.Option.Administrator.removeItem: removeItem
        ]
...
extension VendingMachineController: Administrator {
    func removeItem() {
        inputView.show(Message.Question.Administrator.enterItemNameToRemove)
        let name = inputView.ask(Message.Question.Administrator.itemName)
...
extension VendingMachineController: BeverageMakable {
    func makeBeverage() -> Beverage {
        ...
        let functions = [
            Message.Option.Administrator.manualCreate: makeBeverageManual,
            Message.Option.Administrator.selectInOptions: makeBeverageInOptions
        ]
        
        inputView.show(Message.Question.Administrator.selectCreateOption)
    ...
    func makeBeverageManual() -> Beverage {
        inputView.show(Message.Question.Administrator.enter(target: "이름"))
        let name = inputView.ask("이름")
        
        inputView.show(Message.Question.Administrator.enter(target: "가격"))
        let price = inputView.askNumber("가격")
        
        inputView.show(Message.Question.Administrator.enter(target: "용량"))
        let packageSize = inputView.askNumber("용량")
        
        inputView.show(Message.Question.Administrator.enter(target: "브랜드"))
        let brand = inputView.ask("브랜드")
        
        inputView.show(Message.Question.Administrator.enter(target: "유효기간 일 수"))
        let expirationPeriod = inputView.askDays("유효기간")
        
        inputView.show(Message.Question.Administrator.enter(target: "음료의 온도"))
        let beverageTemperature = inputView.askNumber("온도")
        
        inputView.show(Message.Question.Administrator.enter(target: "생산 날짜"))
    ...
    func makeMilk(dateOfManufacture: Date) -> Milk {
        inputView.show(Message.Question.Administrator.choose(target: "우유의 종류"))
        let option = inputView.askForChoice(options: [BeverageMaker.MilkOption.chocolate, .strawberry], showingBy: ["초코 우유", "딸기 우유"])
        
        inputView.show(Message.Question.Administrator.enter(target: "우유 목장"))
    ...
    func makeSoftDrink(dateOfManufacture: Date) -> SoftDrink {
        inputView.show(Message.Question.Administrator.choose(target: "탄산 음료의 종류"))
    ...
    func makeCoffee(dateOfManufacture: Date) -> Coffee {
        inputView.show(Message.Question.Administrator.choose(target: "커피의 종류"))
