...
class BeverageTest: XCTestCase {
    ...
    func test_IsLowCalorie_ChocoMilk() {
        let chocoMilk = ChocoMilk("서울우유", 200, 1000, "날마다초코우유", Date.init(timeIntervalSinceNow: 0), Date.init(timeIntervalSinceNow: 60*60*24*7), 240, manufacturerCode: 1001, packingMaterial: "종이")
    ...
    func test_IsLowCalorie_Cider() {
        let cider = CiderSoftDrink("롯데칠성음료", 350, 2000, "사이다", Date.init(timeIntervalSinceNow: 0), Date.init(timeIntervalSinceNow: 60*60*24*30*6), 250, carbonContent: 60)
        XCTAssertEqual(cider.isLowCalorie(), false)
    }

    func test_ContainsCarbonicGas_Fanta() {
        let fanta = FantaSoftDrink("코카콜라컴퍼니", 350, 2000, "환타", Date.init(timeIntervalSinceNow: 0), Date.init(timeIntervalSinceNow: 60*60*24*30*7), 300, carbonContent: 40)
    ...
    func test_IsDecaffeinated_Cantata() {
        let cantata = CantataCoffee("롯데칠성음료", 200, 2200, "칸타타", Date.init(timeIntervalSinceNow: 0), Date.init(timeIntervalSinceNow: 60*60*24*14), 250, caffeineLevels: 10, isHot: false, isSweetened: true)
        XCTAssertEqual(cantata.isDecaffeinated(), true)
    }

    func test_IsLowCalorie_Georgia() {
        let georgia = GeorgiaCoffee("코카콜라", 200, 2200, "조지아커피", Date.init(timeIntervalSinceNow: 0), Date.init(timeIntervalSinceNow: 60*60*24*2), 100, caffeineLevels: 25, isHot: true, isSweetened: false)
