...
class VendingMachineUnitTest: XCTestCase {
    ...
    func test_자판기음료객체추가메서드() {
        let baseCountOfStrawBerryMilk = vendingMachineForTest.getInventory()["딸기우유"]?.count ?? 0
        ...
        XCTAssertTrue(vendingMachineForTest.getInventory()["딸기우유"]?.count == baseCountOfStrawBerryMilk + 1)
    }
    ...
    func test_자판기음료구매메서드() {
        vendingMachineForTest.addMoney(10000)
        vendingMachineForTest.buy("딸기우유")
        XCTAssertNil(vendingMachineForTest.getInventory()["딸기우유"])
    }
    ...
    func test_자판기재고메서드() {
        XCTAssertNotNil(vendingMachineForTest.getInventory())
    }
