...
class VendingMachineAdminTest: XCTestCase {
    ...
    func testAdd() {
        ...
        let chocolateMilk = ChocolateMilk.init(brand: "서울우유", weight: 200, price: 1000, name: "초코우유",
                                               dateOfManufacture: "20171207".getDateFromString(),
                                               validate: "20171220".getDateFromString())
    ...
    func testGetInventory() {
        let admin: VendingMachineAdmin = VendingMachineAdmin.init(vendingMachine: vendingMachineMock)
        XCTAssertEqual(admin.getInventory().count, 6)
        var expectedResult: String = "[\"칸타타커피\": [칸타타커피(Kantata) - Kantata, 400ml, 3000원, 칸타타, 20171207]"
        expectedResult += ", \"TOP커피\": [TOP커피(TOPCoffee) - 맥심, 400ml, 3000원, TOP아메리카노, 20171207]"
        expectedResult += ", \"죠지아커피\": [죠지아커피(Georgia) - 코카콜라, 400ml, 3000원, 죠지아, 20171207]"
        expectedResult += ", \"펩시콜라\": [펩시콜라(PepciCoke) - 펩시, 350ml, 2000원, 다이어트콜라, 20171207]"
        expectedResult += ", \"딸기우유\": [딸기우유(StrawberryMilk) - 서울우유, 200ml, 1000원, 딸기우유, 20171207]"
        expectedResult += ", \"바나나우유\": [바나나우유(BananaMilk) - 서울우유, 200ml, 1000원, 날마다바나나우유, 20171207]]"
        XCTAssertEqual(admin.getInventory().description, expectedResult)
    }
    ...
    func testRemove() {
        ...
        expectedResult += ", \"바나나우유\": [바나나우유(BananaMilk) - 서울우유, 200ml, 1000원, 날마다바나나우유, 20171207]]"
        XCTAssertEqual(admin.getInventory().description, expectedResult)
        let kantata = Kantata.init(brand: "Kantata", weight: 400, price: 3000, name: "칸타타",
                                   dateOfManufacture: "20171207".getDateFromString(), hot: true)
