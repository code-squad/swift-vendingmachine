...
class MilkTest: XCTestCase {
    ...
    func testMilkDescription() {
        ...
        let strawberryMilk = Milk.init(brand: "서울우유", weight: 200, price: 1000, name: "날마다딸기우유", dateOfManufacture: dateOfManufacture, validate: validate)
        XCTAssertEqual(strawberryMilk.description, "서울우유, 200ml, 1000원, 날마다딸기우유, 20171211")
        let bananaMilk = Milk.init(brand: "서울우유", weight: 200, price: 1000, name: "날마다바나나우유", dateOfManufacture: dateOfManufacture, validate: validate)
    ...
    func testValidate() {
        ...
        let strawberryMilk = Milk.init(brand: "서울우유", weight: 200, price: 1000, name: "날마다딸기우유", dateOfManufacture: dateOfManufacture, validate: safeValidate)
        XCTAssertTrue(strawberryMilk.validate(with: Date()))

        let notSafeValidate: Date = "20171210".getDateFromString()
        let bananaMilk = Milk.init(brand: "서울우유", weight: 200, price: 1000, name: "날마다바나나우유", dateOfManufacture: dateOfManufacture, validate: notSafeValidate)
